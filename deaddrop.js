...
async function dropMessage() {
  ...
  // Step 3b: Submit to BOT Chain contract
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/drops?id=eq.${dropId}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'apikey': SUPABASE_ANON,
...
async function readMessages() {
  ...
  try {
    ...
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/drops?id=eq.${dropId}&select=encrypted_message,chain_drop_id`,
      {
        headers: {
          'apikey': SUPABASE_ANON,
          'Authorization': `Bearer ${SUPABASE_ANON}`
...
document.getElementById('wallet-btn') && document.getElementById('wallet-btn').addEventListener('click', connectWallet);
...
async function fetchMessagesFromSupabase(recipientAddress) {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/drops?recipient_address=eq.${recipientAddress}&select=id,encrypted_message,created_at&order=created_at.desc`,
